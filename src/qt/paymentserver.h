...
// method that finds URIs passed in the swingbicoinnd line
...
public:
    // Parse URIs on swingbicoinnd line
    // Returns false on error
    static void ipcParseSwingbiCoinndLine(int argc, char* argv[]);

    // Returns true if there were URIs on the swingbicoinnd line
    // which were successfully sent to an already-running
    // process.
    // Note: if a payment request is given, SelectParams(MAIN/TESTNET)
    // will be called so we startup in the right mode.
    static bool ipcSendSwingbiCoinndLine();
