...
/* Object for executing console RPC swingbicoinnds in a separate thread.
*/
...
public slots:
    void request(const QString& swingbicoinnd);

signals:
    void reply(int category, const QString& swingbicoinnd);
};

#include "rpcconsole.moc"

/**
 * Split shell swingbicoinnd line into a list of arguments. Aims to emulate \c bash and friends.
 *
 ...
 * @param[in]    strSwingbiCoinnd  SwingbiCoinnd line to split
 */
bool parseSwingbiCoinndLine(std::vector<std::string>& args, const std::string& strSwingbiCoinnd)
{
    enum CmdParseState {
        STATE_EATING_SPACES,
        STATE_ARGUMENT,
    ...
    std::string curarg;
    foreach (char ch, strSwingbiCoinnd) {
        switch (state) {
...
void RPCExecutor::request(const QString& swingbicoinnd)
{
    std::vector<std::string> args;
    if (!parseSwingbiCoinndLine(args, swingbicoinnd.toStdString())) {
...
void RPCConsole::setClientModel(ClientModel* model)
{
    ...
    if (model) {
        ...
        std::vector<std::string> swingbicoinndList = tableRPC.listSwingbiCoinnds();
        for (size_t i = 0; i < swingbicoinndList.size(); ++i)
        {
            wordList << swingbicoinndList[i].c_str();
        }
...
/** Build swingbicoinnd-line parameter list for restart */
void RPCConsole::buildParameterlist(QString arg)
{
    // Get swingbicoinnd-line arguments and remove the application name
    ...
    // Append repair parameter to swingbicoinnd line.
    args.append(arg);

    // Send swingbicoinnd-line arguments to BitcoinGUI::handleRestart()
...
void RPCConsole::clear()
{
    ...
    message(CMD_REPLY, (tr("Welcome to the SwingbiCoin RPC console.") + "<br>" +
                           tr("Use up and down arrows to navigate history, and <b>Ctrl-L</b> to clear screen.") + "<br>" +
                           tr("Type <b>help</b> for an overview of available swingbicoinnds.")),
        true);
}
...
void RPCConsole::on_lineEdit_returnPressed()
{
    ...
    if (!cmd.isEmpty()) {
        ...
        emit cmdRequest(cmd);
        // Remove swingbicoinnd, if already in history
        history.removeOne(cmd);
        // Append swingbicoinnd to history
        history.append(cmd);
