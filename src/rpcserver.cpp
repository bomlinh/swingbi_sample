...
string CRPCTable::help(string strSwingbiCoinnd) const
{
    string strRet;
    string category;
    set<rpcfn_type> setDone;
    vector<pair<string, const CRPCSwingbiCoinnd*> > vSwingbiCoinnds;

    for (map<string, const CRPCSwingbiCoinnd*>::const_iterator mi = mapSwingbiCoinnds.begin(); mi != mapSwingbiCoinnds.end(); ++mi)
        vSwingbiCoinnds.push_back(make_pair(mi->second->category + mi->first, mi->second));
    sort(vSwingbiCoinnds.begin(), vSwingbiCoinnds.end());

    BOOST_FOREACH (const PAIRTYPE(string, const CRPCSwingbiCoinnd*) & swingbicoinnd, vSwingbiCoinnds) {
        const CRPCSwingbiCoinnd* pcmd = swingbicoinnd.second;
        string strMethod = pcmd->name;
        // We already filter duplicates, but these deprecated screw up the sort order
        if (strMethod.find("label") != string::npos)
            continue;
        if ((strSwingbiCoinnd != "" || pcmd->category == "hidden") && strMethod != strSwingbiCoinnd)
            continue;
#ifdef ENABLE_WALLET
    ...
    if (strRet == "")
        strRet = strprintf("help: unknown swingbicoinnd: %s\n", strSwingbiCoinnd);
...
UniValue help(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "help ( \"swingbicoinnd\" )\n"
            "\nList all swingbicoinnds, or get help for a specified swingbicoinnd.\n"
            "\nArguments:\n"
            "1. \"swingbicoinnd\"     (string, optional) The swingbicoinnd to get help on\n"
            "\nResult:\n"
            "\"text\"     (string) The help text\n");

    string strSwingbiCoinnd;
    if (params.size() > 0)
        strSwingbiCoinnd = params[0].get_str();

    return tableRPC.help(strSwingbiCoinnd);
}
...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop SwingbiCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "SwingbiCoin server stopping";
}
 ...
 * Call Table
 */
static const CRPCSwingbiCoinnd vRPCSwingbiCoinnds[] =
    {
        ...
        /* SwingbiCoin features */
        {"swingbicoin", "masternode", &masternode, true, true, false},
        {"swingbicoin", "listmasternodes", &listmasternodes, true, true, false},
        {"swingbicoin", "getmasternodecount", &getmasternodecount, true, true, false},
        {"swingbicoin", "masternodeconnect", &masternodeconnect, true, true, false},
        {"swingbicoin", "masternodecurrent", &masternodecurrent, true, true, false},
        {"swingbicoin", "masternodedebug", &masternodedebug, true, true, false},
        {"swingbicoin", "startmasternode", &startmasternode, true, true, false},
        {"swingbicoin", "createmasternodekey", &createmasternodekey, true, true, false},
        {"swingbicoin", "getmasternodeoutputs", &getmasternodeoutputs, true, true, false},
        {"swingbicoin", "listmasternodeconf", &listmasternodeconf, true, true, false},
        {"swingbicoin", "getmasternodestatus", &getmasternodestatus, true, true, false},
        {"swingbicoin", "getmasternodewinners", &getmasternodewinners, true, true, false},
        {"swingbicoin", "getmasternodescores", &getmasternodescores, true, true, false},
        {"swingbicoin", "mnbudget", &mnbudget, true, true, false},
        {"swingbicoin", "preparebudget", &preparebudget, true, true, false},
        {"swingbicoin", "submitbudget", &submitbudget, true, true, false},
        {"swingbicoin", "mnbudgetvote", &mnbudgetvote, true, true, false},
        {"swingbicoin", "getbudgetvotes", &getbudgetvotes, true, true, false},
        {"swingbicoin", "getnextsuperblock", &getnextsuperblock, true, true, false},
        {"swingbicoin", "getbudgetprojection", &getbudgetprojection, true, true, false},
        {"swingbicoin", "getbudgetinfo", &getbudgetinfo, true, true, false},
        {"swingbicoin", "mnbudgetrawvote", &mnbudgetrawvote, true, true, false},
        {"swingbicoin", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"swingbicoin", "checkbudgets", &checkbudgets, true, true, false},
        {"swingbicoin", "mnsync", &mnsync, true, true, false},
        {"swingbicoin", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
...
{
    unsigned int vcidx;
    for (vcidx = 0; vcidx < (sizeof(vRPCSwingbiCoinnds) / sizeof(vRPCSwingbiCoinnds[0])); vcidx++) {
        const CRPCSwingbiCoinnd* pcmd;

        pcmd = &vRPCSwingbiCoinnds[vcidx];
        mapSwingbiCoinnds[pcmd->name] = pcmd;
    }
}

const CRPCSwingbiCoinnd* CRPCTable::operator[](string name) const
{
    map<string, const CRPCSwingbiCoinnd*>::const_iterator it = mapSwingbiCoinnds.find(name);
    if (it == mapSwingbiCoinnds.end())
        return NULL;
    return (*it).second;
}
...
{
    // Find method
    const CRPCSwingbiCoinnd* pcmd = tableRPC[strMethod];
    if (!pcmd)
...
std::vector<std::string> CRPCTable::listSwingbiCoinnds() const
{
    std::vector<std::string> swingbicoinndList;
    typedef std::map<std::string, const CRPCSwingbiCoinnd*> swingbicoinndMap;

    std::transform( mapSwingbiCoinnds.begin(), mapSwingbiCoinnds.end(),
                   std::back_inserter(swingbicoinndList),
                   boost::bind(&swingbicoinndMap::value_type::first,_1) );
    return swingbicoinndList;
}

std::string HelpExampleCli(string methodname, string args)
{
    return "> swingbicoin-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:52542/\n";
}
