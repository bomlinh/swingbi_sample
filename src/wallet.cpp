...
/**
 * Fees smaller than this (in uSWBI) are considered zero fee (for transaction creation)
...
void CWalletTx::RelayWalletTransaction(std::string strSwingbiCoinnd)
{
    if (!IsCoinBase()) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SWBI.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-SWBI-address
                    CScript scriptChange;
 ...
 */
bool CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, std::string strSwingbiCoinnd)
{
    {
        LOCK2(cs_main, cs_wallet);
        ...
        wtxNew.RelayWalletTransaction(strSwingbiCoinnd);
    }
    return true;
}
