...
#include <miniupnpc/upnpswingbicoinnds.h>
...
bool CNode::DisconnectOldProtocol(int nVersionRequired, string strLastSwingbiCoinnd)
{
    fDisconnect = false;
    if (nVersion < nVersionRequired) {
        LogPrintf("%s : peer=%d using obsolete version %i; disconnecting\n", __func__, id, nVersion);
        PushMessage("reject", strLastSwingbiCoinnd, REJECT_OBSOLETE, strprintf("Version must be %d or greater", ActiveProtocol()));
        fDisconnect = true;
    }
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SwingbiCoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        if (fDiscover) {
            ...
            if (r != UPNPSWINGBICOINND_SUCCESS)
        ...
        string strDesc = "SwingbiCoin " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SwingbiCoin Core is probably already running."), addrBind.ToString());
        else
...
void CNode::BeginMessage(const char* pszSwingbiCoinnd) EXCLUSIVE_LOCK_FUNCTION(cs_vSend)
{
    ENTER_CRITICAL_SECTION(cs_vSend);
    assert(ssSend.size() == 0);
    ssSend << CMessageHeader(pszSwingbiCoinnd, 0);
    LogPrint("net", "sending: %s ", SanitizeString(pszSwingbiCoinnd));
}
