...
UniValue mnbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        (strSwingbiCoinnd != "vote-alias" && strSwingbiCoinnd != "vote-many" && strSwingbiCoinnd != "prepare" && strSwingbiCoinnd != "submit" && strSwingbiCoinnd != "vote" && strSwingbiCoinnd != "getvotes" && strSwingbiCoinnd != "getinfo" && strSwingbiCoinnd != "show" && strSwingbiCoinnd != "projection" && strSwingbiCoinnd != "check" && strSwingbiCoinnd != "nextblock"))
        throw runtime_error(
            "mnbudget \"swingbicoinnd\"... ( \"passphrase\" )\n"
            ...
            "  vote-many          - Vote on a SwingbiCoin initiative\n"
            "  vote-alias         - Vote on a SwingbiCoin initiative\n"
            "  vote               - Vote on a SwingbiCoin initiative/budget\n"
    ...
    if (strSwingbiCoinnd == "vote" || strSwingbiCoinnd == "vote-many" || strSwingbiCoinnd == "vote-alias") {
        if (strSwingbiCoinnd == "vote-alias")
            throw runtime_error(
                "vote-alias is not supported with this swingbicoinnd\n"
                "Please use mnbudgetvote instead.\n"
            );
        return mnbudgetvote(params, fHelp);
    }

    if (strSwingbiCoinnd == "projection") {
        UniValue newParams(UniValue::VARR);
        // forward params but skip swingbicoinnd
        for (unsigned int i = 1; i < params.size(); i++) {
            newParams.push_back(params[i]);
    ...
    if (strSwingbiCoinnd == "show" || strSwingbiCoinnd == "getinfo") {
        UniValue newParams(UniValue::VARR);
        // forward params but skip swingbicoinnd
        for (unsigned int i = 1; i < params.size(); i++) {
            newParams.push_back(params[i]);
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 6)
        throw runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"SWBI-address\" monthly-payment\n"
            ...
            "5. \"SWBI-address\":   (string, required) SWBI address to send payments to\n"
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 7)
        throw runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"SWBI-address\" monthly-payment \"fee-tx\"\n"
            ...
            "5. \"SWBI-address\":   (string, required) SWBI address to send payments to\n"
            "6. monthly-payment:  (numeric, required) Monthly payment amount\n"
            "7. \"fee-tx\":         (string, required) Transaction hash from preparebudget swingbicoinnd\n"
    ...
    // Check these inputs the same way we check the vote swingbicoinnds:
    // **********************************************************
...
UniValue mnbudgetvote(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() >= 1) {
        ...
        // Backwards compatibility with legacy `mnbudget` swingbicoinnd
        if (strSwingbiCoinnd == "vote") strSwingbiCoinnd = "local";
        if (strSwingbiCoinnd == "vote-many") strSwingbiCoinnd = "many";
        if (strSwingbiCoinnd == "vote-alias") strSwingbiCoinnd = "alias";
    }

    if (fHelp || (params.size() == 3 && (strSwingbiCoinnd != "local" && strSwingbiCoinnd != "many")) || (params.size() == 4 && strSwingbiCoinnd != "alias") ||
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) SWBI address of payment\n"
...
UniValue mnfinalbudget(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        (strSwingbiCoinnd != "suggest" && strSwingbiCoinnd != "vote-many" && strSwingbiCoinnd != "vote" && strSwingbiCoinnd != "show" && strSwingbiCoinnd != "getvotes"))
        throw runtime_error(
            "mnfinalbudget \"swingbicoinnd\"... ( \"passphrase\" )\n"
            "Vote or show current budgets\n"
            "\nAvailable swingbicoinnds:\n"
