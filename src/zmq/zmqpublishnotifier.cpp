...
bool CZMQAbstractPublishNotifier::SendMessage(const char *swingbicoinnd, const void* data, size_t size)
{
    assert(psocket);

    /* send three parts, swingbicoinnd & data & a LE 4byte sequence number */
    unsigned char msgseq[sizeof(uint32_t)];
    WriteLE32(&msgseq[0], nSequence);
    int rc = zmq_send_multipart(psocket, swingbicoinnd, strlen(swingbicoinnd), data, size, msgseq, (size_t)sizeof(uint32_t), (void*)0);
    if (rc == -1)
        return false;
