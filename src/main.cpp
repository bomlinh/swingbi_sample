...
// Copyright (c) 2017-2018 The SwingbiCoin developers
...
#if defined(NDEBUG)
#error "SwingbiCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uSWBI) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1)
        nSubsidy = 2175005 * COIN;
    if (nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 2175000 * COIN; // premine
    else if (nHeight <= 21601)
        nSubsidy = 1 * COIN;
    else if (nHeight > 21601 && nHeight <= 547202)
        nSubsidy = 120 * COIN;
    else if (nHeight > 547202 && nHeight <= 1598403)
        nSubsidy = 60 * COIN;
    else if (nHeight > 1598403 && nHeight <= 3700804)
        nSubsidy = 30 * COIN;
    else if (nHeight > 3700804 && nHeight <= 7905605)
        nSubsidy = 14 * COIN;
    else if (nHeight > 7905605 && nHeight <= 20868083)
        nSubsidy = 0 * COIN;
    else
        nSubsidy = 0 * COIN;
...
{
    RenameThread("swingbicoin-scriptch");
...
bool DisconnectBlocksAndReprocess(int blocks)
{
    ...
    LogPrintf("DisconnectBlocksAndReprocess: Got swingbicoinnd to replay %d blocks\n", blocks);
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                bool pushed = false;
                {
                    ...
                    if (mi != mapRelay.end()) {
                        pfrom->PushMessage(inv.GetSwingbiCoinnd(), (*mi).second);
                        pushed = true;
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strSwingbiCoinnd, CDataStream& vRecv, int64_t nTimeReceived)
{
    RandAddSeedPerfmon();
    if (fDebug)
         LogPrint("net", "received: %s (%u bytes) peer=%d\n", SanitizeString(strSwingbiCoinnd), vRecv.size(), pfrom->id);
    if (mapArgs.count("-dropmessagestest") && GetRand(atoi(mapArgs["-dropmessagestest"])) == 0) {
        LogPrintf("dropmessagestest DROPPING RECV MESSAGE\n");
        return true;
    }

    if (strSwingbiCoinnd == "version") {
        // Each connection can only send one version message
        if (pfrom->nVersion != 0) {
            pfrom->PushMessage("reject", strSwingbiCoinnd, REJECT_DUPLICATE, string("Duplicate version message"));
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 1);
            return false;
        }

        // SWBI: We use certain sporks during IBD, so check to see if they are
        ...
        if (pfrom->DisconnectOldProtocol(ActiveProtocol(), strSwingbiCoinnd))
            return false;
    ...
    else if (strSwingbiCoinnd == "verack") {
    ...
    else if (strSwingbiCoinnd == "getdata") {
        vector<CInv> vInv;
        vRecv >> vInv;
        if (vInv.size() > MAX_INV_SZ) {
            Misbehaving(pfrom->GetId(), 20);
    ...
    else if (strSwingbiCoinnd == "getblocks" || strSwingbiCoinnd == "getheaders") {
    ...
    else if (strSwingbiCoinnd == "headers" && Params().HeadersFirstSyncingActive()) {
        CBlockLocator locator;
        uint256 hashStop;
        vRecv >> locator >> hashStop;
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            pfrom->PushMessage("reject", strSwingbiCoinnd, state.GetRejectCode(),
    ...
    else if (strSwingbiCoinnd == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
    ...
    else if (strSwingbiCoinnd == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
        uint256 hashBlock = block.GetHash();
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            if (!mapBlockIndex.count(block.GetHash())) {
                ...
                pfrom->DisconnectOldProtocol(ActiveProtocol(), strSwingbiCoinnd);
            } else {
    ...
    // getaddr message mitigates the attack.
    else if ((strSwingbiCoinnd == "getaddr") && (pfrom->fInbound)) {
    ...
    else if (strSwingbiCoinnd == "mempool") {
    ...
    else if (fAlerts && strSwingbiCoinnd == "alert") {
        CAlert alert;
        vRecv >> alert;
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
             ...
             (strSwingbiCoinnd == "filterload" ||
                 strSwingbiCoinnd == "filteradd" ||
                 strSwingbiCoinnd == "filterclear")) {
        LogPrintf("bloom message=%s\n", strSwingbiCoinnd);
        Misbehaving(pfrom->GetId(), 100);
    }

    else if (strSwingbiCoinnd == "filterload") {
        CBloomFilter filter;
        vRecv >> filter;
    ...
    else if (strSwingbiCoinnd == "filteradd") {
    ...
    else if (strSwingbiCoinnd == "filterclear") {
        LOCK(pfrom->cs_filter);
        delete pfrom->pfilter;
    ...
    else if (strSwingbiCoinnd == "reject") {
        if (fDebug) {
            try {
                string strMsg;
                unsigned char ccode;
                string strReason;
                vRecv >> LIMITED_STRING(strMsg, CMessageHeader::SWINGBICOINND_SIZE) >> ccode >> LIMITED_STRING(strReason, MAX_REJECT_MESSAGE_LENGTH);
        ...
        mnodeman.ProcessMessage(pfrom, strSwingbiCoinnd, vRecv);
        budget.ProcessMessage(pfrom, strSwingbiCoinnd, vRecv);
        masternodePayments.ProcessMessageMasternodePayments(pfrom, strSwingbiCoinnd, vRecv);
        ProcessMessageSwiftTX(pfrom, strSwingbiCoinnd, vRecv);
        ProcessSpork(pfrom, strSwingbiCoinnd, vRecv);
        masternodeSync.ProcessMessage(pfrom, strSwingbiCoinnd, vRecv);
    }
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_START_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetSwingbiCoinnd()), pfrom->id);
            fOk = false;
            break;
        }
        ...
        if (!hdr.IsValid()) {
            LogPrintf("PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetSwingbiCoinnd()), pfrom->id);
            continue;
        }
        string strSwingbiCoinnd = hdr.GetSwingbiCoinnd();
        ...
        if (nChecksum != hdr.nChecksum) {
            LogPrintf("ProcessMessages(%s, %u bytes): CHECKSUM ERROR nChecksum=%08x hdr.nChecksum=%08x\n",
                SanitizeString(strSwingbiCoinnd), nMessageSize, nChecksum, hdr.nChecksum);
            continue;
        }
        ...
        bool fRet = false;
        try {
            fRet = ProcessMessage(pfrom, strSwingbiCoinnd, vRecv, msg.nTime);
            boost::this_thread::interruption_point();
        } catch (std::ios_base::failure& e) {
            pfrom->PushMessage("reject", strSwingbiCoinnd, REJECT_MALFORMED, string("error parsing message"));
            if (strstr(e.what(), "end of data")) {
                // Allow exceptions from under-length message on vRecv
                LogPrintf("ProcessMessages(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its stated length\n", SanitizeString(strSwingbiCoinnd), nMessageSize, e.what());
        ...
        if (!fRet)
            LogPrintf("ProcessMessage(%s, %u bytes) FAILED peer=%d\n", SanitizeString(strSwingbiCoinnd), nMessageSize, pfrom->id);
...
{
    {
        ...
        if (pingSend) {
            ...
            if (pto->nVersion > BIP0031_VERSION) {
                ...
                // Peer is too old to support ping swingbicoinnd with nonce, pong will never arrive.
