...
// SWBI: on startup load spork values from previous session if they exist in the sporkDB
...
void ProcessSpork(CNode* pfrom, std::string& strSwingbiCoinnd, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all masternode related functionality

    if (strSwingbiCoinnd == "spork") {
        ...
        // SWBI: add to spork database.
        pSporkDB->WriteSpork(spork.nSporkID, spork);
    }
    if (strSwingbiCoinnd == "getsporks") {
