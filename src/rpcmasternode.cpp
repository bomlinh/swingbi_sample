...
// This swingbicoinnd is retained for backwards compatibility, but is deprecated.
// Future removal of this swingbicoinnd is planned to keep things clean.
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        (strSwingbiCoinnd != "start" && strSwingbiCoinnd != "start-alias" && strSwingbiCoinnd != "start-many" && strSwingbiCoinnd != "start-all" && strSwingbiCoinnd != "start-missing" &&
            strSwingbiCoinnd != "start-disabled" && strSwingbiCoinnd != "list" && strSwingbiCoinnd != "list-conf" && strSwingbiCoinnd != "count" && strSwingbiCoinnd != "enforce" &&
            strSwingbiCoinnd != "debug" && strSwingbiCoinnd != "current" && strSwingbiCoinnd != "winners" && strSwingbiCoinnd != "genkey" && strSwingbiCoinnd != "connect" &&
            strSwingbiCoinnd != "outputs" && strSwingbiCoinnd != "status" && strSwingbiCoinnd != "calcscore"))
        throw runtime_error(
            "masternode \"swingbicoinnd\"...\n"
            "\nSet of swingbicoinnds to execute masternode related actions\n"
            "This swingbicoinnd is deprecated, please see individual swingbicoinnd documentation for future reference\n\n"

            "\nArguments:\n"
            "1. \"swingbicoinnd\"        (string or set of strings, required) The swingbicoinnd to execute\n"

            "\nAvailable swingbicoinnds:\n"
            "  count        - Print count information of all known masternodes\n"
            "  current      - Print info on current masternode winner\n"
            "  debug        - Print masternode status\n"
            "  genkey       - Generate new masternodeprivkey\n"
            "  outputs      - Print masternode compatible outputs\n"
            "  start        - Start masternode configured in swingbicoin.conf\n"
    ...
    if (strSwingbiCoinnd == "list") {
        UniValue newParams(UniValue::VARR);
        // forward params but skip swingbicoinnd
    ...
    if (strSwingbiCoinnd == "start" || strSwingbiCoinnd == "start-alias" || strSwingbiCoinnd == "start-many" || strSwingbiCoinnd == "start-all" || strSwingbiCoinnd == "start-missing" || strSwingbiCoinnd == "start-disabled") {
        return startmasternode(params, fHelp);
    }

    if (strSwingbiCoinnd == "genkey") {
        UniValue newParams(UniValue::VARR);
        // forward params but skip swingbicoinnd
        for (unsigned int i = 1; i < params.size(); i++) {
            newParams.push_back(params[i]);
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode SWBI address\n"
...
{
    std::string strSwingbiCoinnd;
    if (params.size() >= 1) {
        strSwingbiCoinnd = params[0].get_str();

        // Backwards compatibility with legacy 'masternode' super-swingbicoinnd forwarder
        if (strSwingbiCoinnd == "start") strSwingbiCoinnd = "local";
        if (strSwingbiCoinnd == "start-alias") strSwingbiCoinnd = "alias";
        if (strSwingbiCoinnd == "start-all") strSwingbiCoinnd = "all";
        if (strSwingbiCoinnd == "start-many") strSwingbiCoinnd = "many";
        if (strSwingbiCoinnd == "start-missing") strSwingbiCoinnd = "missing";
        if (strSwingbiCoinnd == "start-disabled") strSwingbiCoinnd = "disabled";
    }

    if (fHelp || params.size() < 2 || params.size() > 3 ||
        (params.size() == 2 && (strSwingbiCoinnd != "local" && strSwingbiCoinnd != "all" && strSwingbiCoinnd != "many" && strSwingbiCoinnd != "missing" && strSwingbiCoinnd != "disabled")) ||
        (params.size() == 3 && strSwingbiCoinnd != "alias"))
        throw runtime_error(
    ...
    if (strSwingbiCoinnd == "all" || strSwingbiCoinnd == "many" || strSwingbiCoinnd == "missing" || strSwingbiCoinnd == "disabled") {
        if (pwalletMain->IsLocked())
            throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");

        if ((strSwingbiCoinnd == "missing" || strSwingbiCoinnd == "disabled") &&
            (masternodeSync.RequestedMasternodeAssets <= MASTERNODE_SYNC_LIST ||
                masternodeSync.RequestedMasternodeAssets == MASTERNODE_SYNC_FAILED)) {
            throw runtime_error("You can't use this swingbicoinnd until masternode list is synced\n");
        }
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if (pmn != NULL) {
                if (strSwingbiCoinnd == "missing") continue;
                if (strSwingbiCoinnd == "disabled" && pmn->IsEnabled()) continue;
            }
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) SWBI address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) SWBI MN Address\n"
