...
public:
    ...
    /** Send a swingbicoinnd, register a handler for the reply.
     * A trailing CRLF is automatically added.
     * Return true on success.
     */
    bool SwingbiCoinnd(const std::string &cmd, const ReplyHandlerCB& reply_handler);
...
bool TorControlConnection::SwingbiCoinnd(const std::string &cmd, const ReplyHandlerCB& reply_handler)
{
    if (!b_conn)
        return false;
...
void TorController::add_onion_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    ...
    } else if (reply.code == 510) { // 510 Unrecognized swingbicoinnd
        LogPrintf("tor: Add onion failed with unrecognized swingbicoinnd (You probably need to upgrade Tor)\n");
    } else {
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.SwingbiCoinnd(strprintf("ADD_ONION %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
 ...
 *
 *    After a controller sends a successful AUTHCHALLENGE swingbicoinnd, the
 *    next swingbicoinnd sent on the connection must be an AUTHENTICATE swingbicoinnd,
 *    and the only authentication string which that AUTHENTICATE swingbicoinnd
 *    will accept is:
 *
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            _conn.SwingbiCoinnd("AUTHENTICATE " + HexStr(computedClientHash), boost::bind(&TorController::auth_cb, this, _1, _2));
        } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                ...
                _conn.SwingbiCoinnd("AUTHENTICATE \"" + torpassword + "\"", boost::bind(&TorController::auth_cb, this, _1, _2));
            } else {
                LogPrintf("tor: Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint("tor", "tor: Using NULL authentication\n");
            _conn.SwingbiCoinnd("AUTHENTICATE", boost::bind(&TorController::auth_cb, this, _1, _2));
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                // _conn.SwingbiCoinnd("AUTHENTICATE " + HexStr(status_cookie.second), boost::bind(&TorController::auth_cb, this, _1, _2));
                cookie = std::vector<uint8_t>(status_cookie.second.begin(), status_cookie.second.end());
                clientNonce = std::vector<uint8_t>(TOR_NONCE_SIZE, 0);
                GetRandBytes(&clientNonce[0], TOR_NONCE_SIZE);
                _conn.SwingbiCoinnd("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&TorController::authchallenge_cb, this, _1, _2));
            } else {
...
{
    reconnect_timeout = RECONNECT_TIMEOUT_START;
    // First send a PROTOCOLINFO swingbicoinnd to figure out what authentication is expected
    if (!_conn.SwingbiCoinnd("PROTOCOLINFO 1", boost::bind(&TorController::protocolinfo_cb, this, _1, _2)))
        LogPrintf("tor: Error sending initial protocolinfo swingbicoinnd\n");
}
