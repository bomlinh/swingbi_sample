...
*Setup instructions for a gitian build of SwingbiCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SwingbiCoin
...
- [Building SwingbiCoin](#building-swingbicoin)
...
For example to connect as `root` from a Linux swingbicoinnd prompt use
...
user can use the sudo swingbicoinnd. Type/paste the following in the terminal:
...
Clone the git repositories for swingbicoin and gitian and then checkout the swingbicoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the swingbicoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building SwingbiCoin
----------------------

To build SwingbiCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the swingbicoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/swingbicoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/swingbicoin/swingbicoin
...
For example:
```bash
URL=https://github.com/konez2k/swingbicoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit swingbicoin=${COMMIT} --url swingbicoin=${URL} ../swingbicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit swingbicoin=${COMMIT} --url swingbicoin=${URL} ../swingbicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit swingbicoin=${COMMIT} --url swingbicoin=${URL} ../swingbicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/swingbicoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/swingbicoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/swingbicoin-build.assert
```
...
[swingbicoin/gitian.sigs](https://github.com/swingbicoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
