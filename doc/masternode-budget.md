...
SwingbiCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<swingbicoin_address\> \<monthly_payment_swingbicoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ SWBI, _12_ times over the course of a year totaling _24000_ SWBI.

**Warning -- if you change any fields within this swingbicoinnd, the collateral transaction will become invalid.**

Submit proposal to network
------------------------

mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<swingbicoin_address\> \<monthly_payment_swingbicoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` SWBI.


RPC SwingbiCoinnds
------------------------

The following new RPC swingbicoinnds are supported:
