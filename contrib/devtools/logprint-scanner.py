...
def countRelevantSwingbiCoins(line):
    openParensPosStack = []
    openParensPos = 0
    charCounter = 0
    numRelevantSwingbiCoins = 0
    ...
    for char in line:
        ...
        if char == "," and openParensPosStack[-1] == firstOpenParensIndex:
            numRelevantSwingbiCoins += 1
        charCounter += 1

    return numRelevantSwingbiCoins
            ...
            if tempLine.find("LogPrint") != -1:
                if tempLine.count("(") == tempLine.count(")"):
                    ...
                    if havePercents:
                        ...
                        # Count the number of swingbicoins after the format specifier string.  Check to see if it matches the number of format specifiers.
                        # Assumes quotes are not escaped in the specifier string and there are no percent signs when specifying the debug level.

                        # First, determine the position of the swingbicoin after the format specifier section, named swingbicoinAfterEndSpecifierStringIndex
                        ...
                        swingbicoinAfterEndSpecifierStringIndex = tempLine.find(',',endSpecifierStringIndex)

                        # Count the number of swingbicoins after the specifier string
                        line = "(" + tempLine[swingbicoinAfterEndSpecifierStringIndex:-1]
                        numSwingbiCoins = countRelevantSwingbiCoins(line)

                        # Determine number of extra percents after specifier string
                        numExtraPercents = tempLine.count('%', swingbicoinAfterEndSpecifierStringIndex)
                        ...
                        if numPercents != numSwingbiCoins:
                            print "Incorrect number of arguments for LogPrint(f) statement found."
                            print(str(file) + ":" + str(lineCounter - tempCount))
                            print "Line = " + tempLine
                            print("numRelevantSwingbiCoins = " + str(numSwingbiCoins) + ", numRelevantPercents = " + str(numPercents))
                            print ""
